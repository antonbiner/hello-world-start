...
// Ensure HR dropdown has reports and departments. Call after load to add new HR sub-items.
export function ensureHrDropdownItems(): void {
  try {
    ...
    if (!urls.has('/dashboard/hr/attendance/reports')) {
      toAdd.push({ title: 'reports', url: '/dashboard/hr/attendance/reports', description: 'Attendance reports' });
    }
    if (!urls.has('/dashboard/hr/departments')) {
      toAdd.push({ title: 'departments', url: '/dashboard/hr/departments', description: 'Departments & org chart' });
    }
    if (toAdd.length > 0) {
      hrItem.dropdown = [...hrItem.dropdown, ...toAdd];
