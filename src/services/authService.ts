...
class AuthService {
  ...
  private saveUserSession(authResponse: AuthResponse, persistent: boolean = true, loginType: 'admin' | 'user' = 'user'): void {
    ...
    // 🔧 FIX: Initialize tenant for offline sync to work properly
    // If getCurrentTenant() returns null (e.g., on localhost), use email domain or default
    if (!getCurrentTenant() && authResponse.user?.email) {
      const emailDomain = authResponse.user.email.split('@')[1];
      const tenantSlug = emailDomain?.split('.')[0] || 'default';
      storage.setItem('tenant_override', tenantSlug.toLowerCase());
      console.info(`[AUTH] Initialized tenant override: ${tenantSlug} (from email domain: ${emailDomain})`);
    }
