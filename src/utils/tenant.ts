...
function clearLogoCaches(): void {
  ...
  // Also clear user_data stored logo if it exists so it doesn't get re-cached immediately
  try {
...
export function setTenantOverrideWithoutReload(tenant: string | null): void {
  if (tenant) {
    localStorage.setItem(TENANT_OVERRIDE_KEY, tenant);
    cachedTenant = tenant.trim().toLowerCase();
  } else {
    localStorage.removeItem(TENANT_OVERRIDE_KEY);
    cachedTenant = undefined;
    cachedTenant = getTenantFromHostname(); // re-evaluates
  }
  clearLogoCaches();
}
